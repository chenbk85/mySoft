...
{
	if (sc->flags == PTR) {
		struct sb_pointer* sp = (struct sb_pointer *)((char *objp) + sc->object_size - sizeof(*sp));
		if (sp->magic != SB_MAGIC) *(int *)0x0 = 0x123;
		return sp->sb;
	}
	else {
		sb = (slab_block_t* )((unsigned long long)objp & ~PAGE_MASK);
	}
//	slab_block_t* sb = __obj_to_slab(objp);
	return sb;
}
...
static slab_size_t __estimate_slab_size_small(size, slab_size_t* num_p_slab, slab_size_t* slab_size)
{
	assert((size + sizeof(slab_block_t)) <= PAGE_SIZE);
	*slab_size = PAGE_SIZE;
	int up;
	slab_size_t left = __calc_slab_sizes(*slab_size, size, num_p_slab, &up);
	return left;
}
...
static int __create(slab_cache_t* sc, const char* name, slab_size_t size, slab_size_t align, slab_size_t maxfree)
{
	...
	if (size < SMALL)
		__estimate_slab_size_small(size, );
	else {
		size 
...
void* slab_cache_allocate(slab_cache_t *sc)
{
	...
	if (sc->cur != NULL) {
		objp = __slab_cache_allocate_partial(sb = sc->cur, sc);
		assert(objp != NULL);
	}
	...
	if (objp != NULL && sc->flags == PTR) {
		struct sb_pointer *sp = (struct sb_pointer *)((char *)objp + sc->object_size - sizeof(*sp));
		sp->magic = SB_MAGIC(0x34191267);
		sp->sb = sb;
	}
