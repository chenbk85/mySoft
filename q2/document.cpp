...
int document_helper::add(const char* name, const void *data)
{
	...
	void *ndata = document_helper::allocate(*fip, data);
	if (ndata == NULL)
		return -1;	
...
{
	if (fix_doc_ != NULL)
		return fix_doc_->get(name);
	else if (dyn_doc_ != NULL)
		return dyn_doc_->get(name);
	return null_fd;
}
...
void* document_helper::allocate(int type, void *ref)
{
	void *ptr;
	size_t size;
