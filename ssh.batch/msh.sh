...
proc usage { argv0 } {
	send_user "usage: $argv0 \[options] \[ command \[ options ] ]\n"
	send_user "   or: $argv0 --help | -help\n"
	send_user "\n"
	send_user "   Auto login remote hosts one by one, \n"
	send_user "   or execute commands on remote hosts one by one or parallelly.\n"
	send_user "\n"
...
set xprompt "md5ea4458190057cf836aa944fa4b625659"
...
proc call_ssh_host { host } {
	...
	spawn -noecho ssh -l $user -p $port $host $command

	set children($spawn_id) $host
	set hostenv($host,spawn_id) $spawn_id
	lappend spawnids $spawn_id

	debug_output "$spawn_id: $children($spawn_id), -l $user -p $port -h $host $command"
}
		...
		-i "$spawnids" "*assword: " {
			...
			if {[process_ssh_output $host $expect_out(buffer)] == 0} {
				...
				if {$interactive == 0} {
					...
					send -i $expect_out(spawn_id) "echo $xprompt\r"
				}
			}
...
if {[llength $commands] > 0} {
	...
	send_user "TODO: interact parallelly with multiple hosts\n"
	exit 1
}
...
debug_output "Cmd Files: $is_command_file"
