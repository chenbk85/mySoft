...
public:
	...
	dyn_array<SIZE_TYPE, Tp>& operator=(dyn_array<SIZE_TYPE, Tp>& da)
	{
		if (this != &da) {
			this->reset(da.count_, da.allocated_, da.data_);
			da.reset(0, 0, NULL);
...
public:
	...
	posting_data(posting_data& o) : data_(o.data_), ids_(o.ids_) {}
	posting_data& operator=(posting_data& o) { if (this != &o) { data_ = o.data_; ids_ = o.ids_; } return *this; }
public:
...
public:
	//int find(const term_data& term, std::vector<doc_id_type>& hits) const;
	//int between(const term_data& start, const term_data& end, bool inclusive, std::vector<doc_id_type>& hits) const;

//	const posting_data& get(doc_id_type index) const { return postings_[index]; }
//	posting_data& get(doc_id_type index) { return postings_[index]; }

	template <typename _OSTR>
	void dump(_OSTR& out);

//	doc_id_type find_begin(const term_data term) const;
//	doc_id_type find_end(const term_data& term) const;

	posting_data* qsearch(const void* data, doc_id_type& pos);
	const posting_data* qsearch(const void* data, doc_id_type& pos) const;

	const posting_data* get(doc_id_type index) const
	{
		assert(index >= 0 && index < postings_.count());
		return &postings_[index];
	}

	posting_data* get(doc_id_type index) 
	{
		assert(index >= 0 && index < postings_.count());
		return &postings_[index];
	}
private:
	const fieldinfo* fi_;
	dyn_array<doc_id_type, posting_data> postings_;
};

template <typename _OSTR>
void posting_table::dump(_OSTR& out)
{
	...
	out << ", count: " << postings_.count() << std::endl;
	for (doc_id_type i = 0; i < postings_.count(); ++i) {
		posting_data& pd = postings_[i];
		out << "\t";
		switch (fi_->data_type()) {
		case TDT_LONG:
...
{
	doc_id_type pos;
	posting_data* pd = qsearch(data, pos);
	if (pd == NULL) {
		if (postings_.insert(pos) < 0)
			return -1;
		pd = &postings_[pos];
		return pd->set(data, did);
	}
...
#if 0
int posting_table::find(const term_data& term, std::vector<doc_id_type>& hits)
{
	doc_id_type first = find_begin(term);
	if (first == NID)
		return -1;
	doc_id_type end = find_end(term);

	for (doc_id_type i = first; i < end; ++i) {
		const posting_data& pd = get(i);
		for (doc_id_type j = 0; j < pd.count(); ++j) {
			hits.push_back(pd.ids(j));
		}
	}
...
int posting_table::between(const term_data& start, const term_data& end, bool inclusive, std::vector<doc_id_type>& hits) const
{
	doc_id_type first = find_begin(start);
	if (first == NID)
		return -1;
	doc_id_type end = find_end(end);

	for (doc_id_type i = first; i <= last; ++i) {
		const posting_data& pd = get(i);
		for (doc_id_type j = 0; j < pd.count(); ++j) {
			hits.push_back(pd.ids(j));
		}
