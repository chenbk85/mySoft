...
public:
	...
	bool operator<(const vKey& o) const {
		return *(unsigned long*)v_ < *(unsigned long*)o.v_;
	}
private:
	const void *v_;
};
...
public:
	dKey() {}
	dKey(const void* data) : v_(data) {}
	bool operator<(const dKey& o) const {
		return *(const double *)v_ < *(const double *)o.v_;
	}
private:
	const void* v_;
};
...
struct posting_btree_traits
{
	...
	static int const leafslots = 1024;
	static int const innerslots = 1024;
};
...
void posting_table::optimize()
{
	if (fi_->data_type() == TDT_LONG || fi_->data_type() == TDT_DOUBLE) {
		postings_.reserve(postings_v_.size());
		vTable::iterator end = postings_v_.end();
		for (vTable::iterator iter = postings_v_.begin(); iter != end; ++iter) {
			postings_.push_back(iter->second);	
		}
		...
		postings_.reserve(postings_v_.size());
		sTable::iterator end = postings_s_.end();
		for (sTable::iterator iter = postings_s_.begin(); iter != end; ++iter) {
			postings_.push_back(iter->second);
		}
...
private:
	dyn_array<doc_id_type, document*> docs_;
...
#else
	docs_.push_back(const_cast<document *>(doc));
#endif

	return 0;
}

void index_data::optimize()
{
	std::vector<int> next_fno(docs_.count(), 0);
	for (int i = 0; i < fis_.count(); ++i) {
		struct timeval t1, t2, t3;
		const fieldinfo& fi = fis_.get(i);

		gettimeofday(&t1, NULL);

		posting_table& pt = get_posting_table(fi.name());
		size_t docs_size = docs_.count();
		for (size_t j = 0; j < docs_size; ++j) 
		{
			if (next_fno[j] < docs_[j]->field_count()) {
				field_data& fd = docs_[j]->get(next_fno[j]);
				if (fd.index_ == i) {
					pt.add(fd.data_, j);
					++next_fno[j];
				}
		...
		fprintf(stderr, "create btree for %s cost: %.3fms\n", fi.name(), ms);
	
		pt.optimize();
		gettimeofday(&t3, NULL);
		ms = (t3.tv_sec - t2.tv_sec) * 1000.0 + (t3.tv_usec - t2.tv_usec) / 1000.0;
		fprintf(stderr, "optimize btree for %s cost: %.3fms\n", fi.name(), ms);
	}
