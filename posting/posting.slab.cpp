...
typedef unsigned short doc_size_type;
...
interface document {
	...
	const_field_iterator begin() const;
	const_field_iterator end() const;

	byte* field(field_id_type fid);
	void field(field_id_type fid, byte* data, doc_size_t dsize);
};

// data format 0
//    the average count of fields for documents is near the total fields count.
//    data are allocated from slab_malloc 
class document_de {
public:
	document();
private:
...
// data format 2:
//    the average count of fields for documents is near the total fields count.
//    data are within document.
class document_se {
public:
	document(field_id_type count, doc_size_type dsize);
private:
	field_id_type count_;
	byte* fields_[0];
};
...
// data format 3:
//   the average count of fields for documents is far less than the total fields
//   data are allocated from slab_malloc
class document_ds {
public:
	document();
...
//	int add(field_id_type index, const void *data);
	...
	struct field_data get(field_id_type index) const { return fields_[index]; }
	...
	int add(std::map<int, std::pair<int, void*> >& fs)
	{
		size_t t = fs.size() * sizeof(short) * 2;
		for (std::map<int, std::pair<int, void*> >::interator iter = fs.begin();
			iter != fs.end();
				++iter) {
			if (iter->second.first == TDT_LONG || iter->second.first == TDT_DOUBLE) {
				t += sizeof(long);
			}
			else {
				t += strlen((char *)iter->second.second) + 1;
			}
		}

		fos_ = (short *)malloc(t);
		size_t off = fs.size() * sizeof(short) * 2;
		char *dst = (char *)fos_ + off;
		for (std::map<int, std::pair<int, void*> >::interator iter = fs.begin();
			iter != fs.end();
				++iter) {
			fos_[i] = iter->first;
			fos_[i+1] = off;
			if (iter->second.first == TDT_LONG || iter->second.first == TDT_DOUBLE) {
				memcpy(dst, iter->second.second, sizeof(long));
				dst += sizeof(long);
				off += sizeof(long);
			}
			else {
				t += strlen((char *)iter->second.second) + 1;
				memcpy(dst, iter->second.second, t);
				dst += t;
				off += t;
			}
		}
private:
	...
	// data format 2:
	//    the average count of fields for documents is near the total fields count.
	//    data are with document (hard to modify fields).
	field_id_type count_;
	byte* fields_[0];
#if DFT=3
	// data format 3:
	...
	struct field_offset fields_[0];
#else
	#error You must define DFT=1, 2, 3, or 4.
#endif
};
...
struct posting_btree_traits
{
	...
	static int const leafslots = 8; //32; //64; //256; //1024;
	static int const innerslots = 8; //32; //64;//256; //1024;
//1024: 384#/s
//256: 844#/s
//64: 1196/s
//32: 1270/s
...
public:
	doc_id_type next_doc_id() { return ++next_doc_id_; }
	void next_doc_id(doc_id_type ni) { next_doc_id_ = ni; }
...
if (t1.tv_sec == 0) gettimeofday(&t1, NULL);
	...
	document* doc = slab_object<document>::instance()->allocate();
	char line[8192];
...
//std::cout << "********** add doc: " << doc->field_count() << std::endl;
			...
			doc = slab_object<document>::instance()->allocate();
		}
		else {
...
//std::cout << "add long: " << name << " = " << *l << std::endl;
				...
				double *d = slab_object<double>::instance()->allocate();
				*d = strtod(vptr,  NULL);
//std::cout << "add double: " << name << " = " << *d << std::endl;
				doc->add(id, d);
			}
			else if (fi->data_type() == TDT_STRING) {
				char *s = (char *)slab_malloc(len - (vptr - line) + 1);
				if (s == NULL) {
//					fprintf(stderr, "malloc for (%s), size=%ld errno=%d\n", vptr, (long)(len - (vptr - line) + 1), errno);
					s = (char *)malloc(len - (vptr - line) + 1);
				}
...
int main(int argc, char **argv)
{
	...
	slab_cache_init(10, NULL, 0, NULL, NULL);
